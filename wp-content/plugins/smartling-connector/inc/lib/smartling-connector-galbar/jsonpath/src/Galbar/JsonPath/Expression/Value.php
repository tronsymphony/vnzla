...
namespace Smartling\Vendor\JsonPath\Expression;
use Smartling\Vendor\JsonPath\Language;
use Smartling\Vendor\JsonPath\Operation;
    ...
    public static function evaluate(&$root, &$partial, $expression)
    {
        if ($expression === Language\Token::VAL_NULL) {
            ...
            if ($expression[0] === Language\Token::ROOT){
                list($result, $_) = \Smartling\Vendor\JsonPath\JsonPath::subtreeGet($root, $root, $expression);
            }
            else if ($expression[0] === Language\Token::CHILD) {
                $expression[0] = Language\Token::ROOT;
                list($result, $_) = \Smartling\Vendor\JsonPath\JsonPath::subtreeGet($root, $partial, $expression);
            }
            if ($result !== false) {
                if ($length) {
