...
$authProvider = \Smartling\Vendor\Smartling\AuthApi\AuthTokenProvider::create($userIdentifier, $userSecretKey);

/**
 * @param \Smartling\Vendor\Smartling\AuthApi\AuthApiInterface $authProvider
...
function listJobsDemo($authProvider, $projectId)
{
    ...
    $response = [];
    $jobs = \Smartling\Vendor\Smartling\Jobs\JobsApi::create($authProvider, $projectId);
    $st = microtime(true);

    try {
        $params = new \Smartling\Vendor\Smartling\Jobs\Params\ListJobsParameters();
        $params->setStatuses([ \Smartling\Vendor\Smartling\Jobs\JobStatus::AWAITING_AUTHORIZATION, \Smartling\Vendor\Smartling\Jobs\JobStatus::IN_PROGRESS,
        ]);
        $response = $jobs->listJobs($params);
    } catch (\Smartling\Vendor\Smartling\Exceptions\SmartlingApiException $e) {
...
function createJobDemo($authProvider, $projectId)
{
    ...
    $result = FALSE;
    $jobs = \Smartling\Vendor\Smartling\Jobs\JobsApi::create($authProvider, $projectId);
    $params = new \Smartling\Vendor\Smartling\Jobs\Params\CreateJobParameters();
...
function updateJobDemo($authProvider, $projectId, $jobId)
{
    ...
    $result = FALSE;
    $jobs = \Smartling\Vendor\Smartling\Jobs\JobsApi::create($authProvider, $projectId);
    $params = new \Smartling\Vendor\Smartling\Jobs\Params\UpdateJobParameters();
...
function cancelJobDemo($authProvider, $projectId, $jobId)
{
    ...
    $jobs = \Smartling\Vendor\Smartling\Jobs\JobsApi::create($authProvider, $projectId);
    $params = new \Smartling\Vendor\Smartling\Jobs\Params\CancelJobParameters();
...
function searchJobDemo($authProvider, $projectId, $fileUri)
{
    ...
    $jobs = \Smartling\Vendor\Smartling\Jobs\JobsApi::create($authProvider, $projectId);
    $info = FALSE;
    $searchParameters = new \Smartling\Vendor\Smartling\Jobs\Params\SearchJobsParameters();
