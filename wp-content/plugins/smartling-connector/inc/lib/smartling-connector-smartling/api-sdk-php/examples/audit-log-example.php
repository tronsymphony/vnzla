...
{
    $response = false;
    $auditLog = \Smartling\Vendor\Smartling\AuditLog\AuditLogApi::create($authProvider, $projectId);
    $st = microtime(true);

    try {
        $searchParams = (new \Smartling\Vendor\Smartling\AuditLog\Params\SearchRecordParameters())
            ->setSearchQuery('clientData.foo:bar')
            ->setOffset(0)
            ->setLimit(100)
            ->setSort('actionTime', \Smartling\Vendor\Smartling\AuditLog\Params\SearchRecordParameters::ORDER_DESC);
...
function createProjectLevelLogRecordDemo($authProvider, $projectId)
{
    ...
    try {
        $createParams = (new \Smartling\Vendor\Smartling\AuditLog\Params\CreateRecordParameters())
            ->setActionTime(time())
            ->setActionType(\Smartling\Vendor\Smartling\AuditLog\Params\CreateRecordParameters::ACTION_TYPE_UPLOAD)
            ->setFileUri("file_uri")
            ->setFileUid("file_uid")
